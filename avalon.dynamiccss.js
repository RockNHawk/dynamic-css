...
(function (avalon, $, win, doc) {
    ...
    avalon.bindingHandlers.dynamiccss = function (data, vmodels) {
        ...
        if (!url) {
            //debugger;
            var code = el.tagName != "STYLE" ? el.innerText : ie ? styleElement.cssText : styleElement.textContent;
            if (code)
                avalon.parseExprProxy(code, vmodels, data, avalon.scanExpr(code)); // parseExprProxy 这个是 avalon 内部方法（需要对 avalon js 源码做一句改动，把那个内部方法暴露出来，avalon 第 3191 行）
            return;
        }
        ...
        $.get(url, function (code) {
            ...
            if (onload)
                onload.apply(styleElement, [code]);
            ...
            if (onerror)
                onload.apply(styleElement, arguments);
        });
    };
...
})(window.avalon, jQuery, window, document); // 这里并不是必须依赖 jQuery ，只是用了 jQuery 的 $.get ，如果你的项目不想引入 jQuery，也是可以的，传入 具有相同 get 方法功能的对象即可
//# sourceMappingURL=avalon.dynamiccss.js.map
